...
- **Audio:** `static/rain-long.m4a`（原 `rain.m4a` 重复 11 次拼接，192kbps 立体声 48kHz，30 分钟，~42MB，streaming 加载）
...
- **修复：** 将原始 `rain.m4a`（2.75 分钟）重复拼接 11 次生成 `rain-long.m4a`（30 分钟），192kbps 立体声 48kHz（~42MB，streaming 加载无影响）。循环间隙 30 分钟才出现一次，用户几乎不会感知
