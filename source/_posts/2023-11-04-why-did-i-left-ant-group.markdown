...
⚠️声明：我在此声明，撰写的文章中的描述仅代表我个人的经历和感受的总结和反思，而不是要对公司或团队发表任何指责或批评。如果在我撰写的文章中有任何不合适或误导性的观点，我深感抱歉，并请立即联系我以便进行修正或删除。
...
# 一、关于我
17 年大学毕业后，只身来到上海，寻觅了一份[创业公司后端开发](/blog/20170223/python-backend-inteview-summery/)的工作。

记得某天工作整整满一年时，收到来自蚂蚁第一任老板的邮件（博客吸引，可以帮忙内推）,便顺水推舟进入面试，并最终加入蚂蚁，成为了一名业务 SRE（[Site Reliability Engineer](/blog/20180403/impressions-of-google-sre/)）。

主要参与国际支付业务线上稳定性保障，通过软件工程等实践在监控告警领域持续探索。

<u>p.s. 为什么离开创业公司加入蚂蚁？</u>
记得面试阶段，对方提问：你维护的业务线上一共维护了几台机器？当时我掰着手指回答后，对方笑了一下，兴奋的说有没有兴趣加入蚂蚁，一起管理上万台服务器（当然加入后，机器也轮不到我们维护 :P）
...
high performance 与晋升，对应经济收入的指数型增长。可能难以想象，相比第一份工作，年收入增长超过了十倍～

### 2）专业的同事
作为每天共事的同事，其对应的专业程度，是工作幸福感的巨大因素之一。

而蚂蚁的同事，无论是个人学历资质，还是工作负责程度，都令人无可挑剔。
...
分享一个津津乐道的小故事：记得一次线上会员系统的数据库抖动了 30s 后自动恢复，但导致某个支付咨询业务的成功量下跌超过 30%，触发了 P1 故障。故障复盘时，所有人的焦点都集中在数据库的稳定性优化，但大家都忽略了一点：一条成熟的支付链路为什么对会员系统的数据库存在强依赖？？也就是说在支付链路中，理论上即使数据库出现问题，99% 的请求也应该由缓存处理，不应该受到影响。

所以我花了一周时间，阅读了会员系统的 java 源码，终于发现其提供的类似 `get_or_create` 的服务，在一开始准备上下文 thread local 的过程中，直接请求了数据的当前时间，用于后续实际触发注册的输入。所以即使用户信息已注册，并存储在缓存之中，还是会依赖数据库。

于是灵机一动，参考之前 Django 框架中 ORM “懒加载” 的设计，对获取数据库当前时间的入口进行了改造（考虑系统内其他 rpc 服务都可以规避数据库强依赖问题），最终成功提交 PR 并发布。

更进一步，面向未来，如何避免该系统的代码改动，以及其他系统避免该问题呢？除了通过日志 tracing 的关联和离线清洗产出 T+1 的报表进行预警，还可以通过定期的自动化线上回归演练，来尽早规避该问题。

---

虽然不像创业公司，每天可以接触新的有趣技术玩物，在蚂蚁却更像一个创业团队，锻炼如何将一件事“做成”的能力：业务背景 -> 定义问题 -> 大的逻辑 -> 技术落地 -> 量化度量 -> 未来规划..

举个不恰当的例子，每个财年开始后，每天工作时都会思考一个问题，也就是如何在你下个晋升答辩中，如何将今年的故事说的更加生动，更加打动评委。

当然如果每个“小P”都需要不断思考自己从事工作的业务价值，不难理解对公司整体业务达成有益，但对于个人来说，除了掉头发，也会有一些负面的影响。

## 负面因素

逻辑：资本剥削 -> 优绩至上 -> ^^个人市场竞争力^^下滑
...
### 2）优绩至上
业务价值：记得 p5 年中绩效考核沟通时，老板反馈其他都不错，就是业务价值需要进一步提升。

但业务价值，也就是 kpi 至上，难免会带来一定副作用：

#### 短视 & 不专业
由于绩效考核的周期为一年，不难理解绝大部份的项目存活周期不超过一年。

同时也不难理解网传的很多大力出奇迹的故事。记得公司鼎盛大力招人的时候，全组技术的人员吃完晚饭，就闭关会议室直至十一二点。只为找简历面试，甚至每周简历捞取数量不达标，会进行罚款。
...
- 参考特征：`buff 叠加`，`兴趣驱动`，`市场需求`、...
- 目标：
    - 运维能力（通用技术栈，例如 AWS）
    - 编程能力（回馈开源社区）
    - 外语能力（流畅无障碍的口语交流）
- 风险：
    - 当然有一天可能会发现不是一个最佳选择，个人只是喜欢钱，而不是写代码 ：）
