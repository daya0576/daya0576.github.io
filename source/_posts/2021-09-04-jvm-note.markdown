---
title: Java 内存管理 - 学习小记
date: 2021-09-04 16:31:21
tags: java
---


> 当前公司技术栈中 Java 是一等公民👍，所以从去年开始被迫营业，从头开始了解和学习这门语言。
>
> 本篇文章是阅读《深入理解 JVM 虚拟机》的读书小记，主要通过绘图与分享的方式，加深自己的理解。

## 引言
在过去三年 SRE 的经历中，遇到过多起因为 JVM OOM 导致的线上故障。其中印象最深的一次排查经历：收到故障外呼后，几个大男人现场梳理业务链路，经过一番排查，最后发现根因竟然是部分网关应用动态加载数据库中的 groovy 脚本，导致 `Metaspace out of memory` 报错，影响了部分 XX 商户的代扣业务，最终落了一个 P4 故障。
...
所以花了一点时间，尝试通过「堆」和「栈」两个视角，将 java8 的内存分布重新绘制一遍（如果有不对的地方辛苦帮忙指正）：
![](/images/blog/2021-09-04-jvm-note/16307787626886.jpg)

 
### 关键点说明
#### 1. 关于 Metaspace
为了解决持久代内存溢出 & 不同虚拟机融合等目的，持久代（PermGen）在 1.8 以后被 Metaspace 取代。

我个人理解最大不同在于：1.8 之前，持久代与 Heap & Stack 都归属**虚拟机内存**，而 Metaspace 侧使用的**本地内存**（native memory），**默认不做限制**。

但是我们通常还是习惯设置 `-XX:MaxMetaspaceSize` 参数，类占据的空间还是有可能超过指定的空间大小，造成`java.lang.OutOfMemoryError: Metaspace` 异常  ，最终导致文章开头说的那个故障。

#### 2. 关于 Stack Frame 中的数据
