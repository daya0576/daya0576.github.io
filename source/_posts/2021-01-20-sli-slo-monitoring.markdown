...
这篇文章将分享两年后，我对 SRE 与 DevOps 的理解；同时结合个人与监控的故事，谈谈如何定义更加饱满的 SLI 来实现预期的 SLO，让 SRE 吃的更香睡的更好 XD
...
### 可观测指标
「可观测指标」可分为三个维度（参考《[Metrics, tracing, and logging](https://peter.bourgon.org/blog/2017/02/21/metrics-tracing-and-logging.html)》），从上往下代表对存储容量的需求，同时对异常检测规则的精度要求也更高：
...
通过发挥 Metric, Tracing, Logging 各自的优势，定义出更加“饱满”的精准 SLI，减少告警误报漏，达到预设的 SLO，最终让 SRE 睡的更香～
...
本文通过过去两年多的线上监控经历，总结出个人视角 SLI 的最佳实践。
