...
1. **项目初始化**阶段：模型天生倾向于一口吃成胖子 ------ 一次性实现所有功能。但假如干到一半留了个烂摊子，下个“人”只能靠“猜”来继续完成工作。
2. **项目快速迭代**阶段：每次开启新 session 后，之前的上下文会完全丢失。想象你每隔一天都招聘一个新员工来帮你编程干活。

基于上面两个问题，文章分别提出了 `Initializer agent` & `Coding agent`。下面尝试用一小段“伪代码”说明逻辑：
...
while True:
    ...
    # 3. 重复上面 Initializer agent 中的 第二步
    # 4. 关闭 session - 保持 clean state
```

通过官方的 demo 运行一个小时，烧掉大几十刀 token 后，个人的感受和问题：
- 虽然项目进展非常低效，例如 agent 在 git init 时，因为本身处在一个大仓库中的问题卡了很久。但神奇的是，每次“卡住”，磕磕绊绊许久，问题都最终被解决了。。
