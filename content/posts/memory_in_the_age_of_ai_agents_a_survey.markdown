...
自从将基于 pi 构建的 iMessage 机器人加入家庭群聊后，最近每天晚上都会一起闲聊数小时。

由于记忆系统设计较为简单，逐渐遇到一些新的问题：例如 1）开启新 session 后，历史聊天记录虽然持久化保存，如何让 agent 高校主动检索 2）可解释性：尝试让 agent 阅读我的博客认识家庭成员，成功总结写入`MEMORY.md`，但忘记了出处 3）记忆冲突：不同时间点的记忆出现冲突 ...

P.S. pi-imessage 中的简易版记忆：
```
# system prompt
...
![](/images/blog/global/17749467711476.jpg)

### 1）原始 token

例如文本（text token），音频，图片等信息，又可以分为三类：一维、二维、和三维。
...
分为两类：
1. 修改模型本身的参数：在训练时将个性化知识直接烧进权重，分为预训练、中间训练、后训练（微调），缺点在于无法更新。 
2. 外挂 LoRA 模块进行自定义，灵活但增加了复杂度。

### 3）模型潜在向量（Latent Memory）
...
相比于原始 token，潜在向量的**不可解释性**反而有助于隐私的保护，处理效率更高，并在文字语言难以描述的信息具备一定优势。
...
1. 事实库（agent 知道什么？）：通过一系列**客观事实**，保证 agent 在交互中的一致性、连贯性、适应性。又可分为：
...
![](/images/blog/global/17749472377985.jpg)

## Dynamics - 记忆的生命周期

生命周期：记忆形成 -> 记忆演化（例如冲突管理，遗忘机制）-> 记忆检索。
