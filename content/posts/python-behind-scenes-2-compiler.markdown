...
p.s. IR 的全称为 intermediate representation

## 总览

上图中编译器由两大块组成，frontend 接收 Python 代码生成 AST，backend 进而转化为 code object。所以从某种意义上来说，CPython 编译器又可以称之为“code object 生成器”。

P.S. 图中 `parser` 也检查 Python 代码的语法错误（语法具体是什么将在下文详细展开）：
...
Python 是一门复杂的语言，但仅有 63 个 TOKEN（[Grammar/Tokens](https://github.com/python/cpython/blob/0bbaf5de9744ae1acea3e2c9ad2257d1cc68e847/Grammar/Tokens#L1)）。
...
新 parser 的生成流程中，跳过了 parse tree，直接生成 AST 树。
...
语法抽象树 Abstract syntax tree (AST) 使用树状结构，作为源代码的一种高级表示：
...
`NUMBER token` 的文本值是如何被转化为数字的？在 ToyParser 示例中，grammar action 可以直接计算结果；而在 CPython 中，action 更多用于构造 AST 节点。
```
...
以赋值语句为例（`x = 1`），它对应的 AST 节点为 `Assign(targets=[Name(id='x', ctx=Store())], value=Constant(value=1))`。如果将它转化为 bytecode，我们需要 `LOAD_CONST` 指令存常量，用 `STORE_NAME` 指令存变量名。但问题在于，赋值时变量 `x` 可能是个方法内的局部变量（`STORE_FAST`），也可能是全局变量（`STORE_GLOBAL`），以及闭包变量（`LOAD_DEREF` / `STORE_DEREF`）。
